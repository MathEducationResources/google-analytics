...
def add_to_target_dict(line, target_dict):
    page, pageviews, unique_pageviews, unique_visitors, avg_time, pages_per_visit, visit_duration = line.strip(
    ).split(',')
    ...
    for exam in list_exams(page_info, course):
        try:
            ...
            for question in list_questions(page_info, course, exam):
                front_and_subpages_dist[
                    exam] += page_info[course][exam][question]['visit_duration']
        except ValueError:
            continue
        except KeyError:
            continue
...
def plot_visit_duration_dist(page_info, course, style='pie', include_subpages=True):
    ...
    for num, key in enumerate(sorted(dict_to_plot.keys())):
        ...
        if 'pie' == style:
            my_labels.append(key.replace('_', '') + ' [' + str(int(y[num] / 60 / 60)) + 'h ' + str(
                int(100 * round(float(dict_to_plot[key]) / sum(dict_to_plot.values()), 2))) + '%]')
        elif 'bar' == style:
            my_labels.append(key.replace('December_', 'Dec').replace('April_', 'Apr').replace(
...
def plot_question_dist(mydict, course, exam, mykey, plot_type='bar'):
    ...
    if len(question_keys) > 1:
        ...
        if 'bar' == plot_type:
            plt.bar(
                range(len(y)), y, align='center', color=create_RGB_list(len(y)))
            plt.gca().set_xticks(range(len(y)))
            plt.gca().set_xticklabels(my_labels)
...
def plot_pie_chart_time_spent_on_course(my_dict, thres, colors):
    ...
    for num, course in enumerate(list_courses(my_dict)):
        ...
        labels.append(
            course + " [" + str(round(total_time[num] / 60.0 / 60.0 / 1000, 1)) + " kh]")
    ...
    for lab in other_labels:
        labels.remove(lab)
    labels.append(
        'OTHER [' + str(round(other_time / 60.0 / 60.0 / 1000, 1)) + ' kh]')
    ...
    plt.title('Total time users spent on all wiki pages by course ' +
              " [" + str(round(np.sum(total_time) / 60.0 / 60.0 / 1000, 1)) + " kh]", size='26')
...
def plot_total_clicks_time_series(date_list, num_clicks, logarithmic, showExamLabels):
    ...
    plt.gca().set_xticklabels(['Jan 12', 'May 12', 'Sep 12', 'Jan 13',
                               'May 13', 'Sep 13', 'Jan 14', 'May 14', 'Sep 14'], size='x-large')
