...
def add_to_target_dict(line, target_dict):
    (page, pageviews, unique_pageviews, unique_visitors,
        avg_time, pages_per_visit, visit_duration) = line.strip().split(',')
...
def add_page_info(page_info, line, which_type):
    if 'exam' == which_type:
        ...
        page_info = add_question_info(
            page_info, line, course_name, exam_name, question_name)
    return page_info
...
def list_exams(page_info, course):
    ...
    for key in sorted_keys:
        if type(page_info[course][key]) is dict:
...
def list_questions(page_info, course, exam):
    ...
    for key in sorted_keys:
        if type(page_info[course][exam][key]) is dict and 'Question' in key:
    ...
    for exam in list_exams(page_info, course):
        try:
            ...
            for question in list_questions(page_info, course, exam):
                front_and_subpages_dist[
                    exam] += page_info[course][exam][question][
                    'visit_duration']
        except ValueError:
            continue
        except KeyError:
...
def data_to_dict_clickdates_clickscount(filename):
    ...
    for line in f:
        if line_type(line) == 'header':
            continue
        elif (line_type(line) == 'exam' or
              line_type(line) == 'course' or
              line_type(line) == 'question'):
...
def is_valid_exam(exam):
    ...
    # IF all above cond. are satisfied, return TRUE
...
def fraction_course_exam_question_by_key(mydict, mykey):
    ...
    for course in list_courses(mydict):
        ...
        for exam in list_exams(mydict, course):
            ...
            for question in list_questions(mydict, course, exam):
                try:
                    total_question_time += mydict[
                        course][exam][question][mykey]
                except KeyError:
                    pass
    total_time = float(
        total_course_time + total_exam_time + total_question_time)
    return (total_course_time / total_time,
            total_exam_time / total_time,
            total_question_time / total_time)
...
def plot_question_dist(mydict, course, exam, mykey, plot_type='bar'):
    ...
    if len(question_keys) > 1:
        ...
        for question in question_keys:
            ...
            # disregard pages with less than too few unique pageviews. High
            # chance these are typos
            ...
            if 'pageviews' == mykey:
                ...
                label_data = '[' + str(int(value / 60)) + \
                    'm' + str(int(value % 60)) + 's]'
                ylabel = 'Average time spent in m'
                y[num] /= 60
            elif 'pages_per_visit' == mykey:
                NotImplemented
            elif 'visit_duration' == mykey:
                label_data = '[' + str(int(value / 60 / 60)) + \
                    'h' + str(int((value % (60 * 60)) / 60)) + 'm]'
        ...
        if 'bar' == plot_type:
            plt.bar(
                range(len(y)), y,
                align='center', color=create_RGB_list(len(y)))
            plt.gca().set_xticks(range(len(y)))
            plt.gca().set_xticklabels(my_labels)
...
def get_total_exam_time(mydict):
    ...
    for course in list_courses(mydict):
        ...
        for exam in list_exams(mydict, course):
            ...
            for question in list_questions(mydict, course, exam):
                total_exam_time[course][
                    exam] += mydict[course][exam][question]['avg_time']
...
# Plot a pie chart showing the relative amount of time spent on pages
# related to a course
...
def plot_pie_chart_time_spent_on_course(my_dict, thres, colors):
    ...
    for num, course in enumerate(list_courses(my_dict)):
        ...
        for exam in list_exams(my_dict, course):
            ...
            for question in list_questions(my_dict, course, exam):
                total_time[
                    num] += my_dict[course][exam][question]['visit_duration']
        labels.append("%s [%.1f kh]" %
                      (course, total_time[num] / 60.0 / 60.0 / 1000))
    ...
    labels.append(
        'OTHER [' + str(
            round(other_time / 60.0 / 60.0 / 1000, 1)) + ' kh]')
    tt = np.append(tt, other_time)

    plt.pie(tt, labels=labels, shadow=False, autopct='%.1f%%',
            pctdistance=0.6, labeldistance=1.2, colors=colors)
    plt.title('Total time users spent on all wiki pages by course ' +
              " [" + str(
                  round(np.sum(total_time) / 60.0 / 60.0 / 1000, 1)) + " kh]",
              size='26')
...
def plot_total_clicks_time_series(date_list, num_clicks,
    ...
    x_axis.append(date_list.index('5/1/14'))
    x_axis.append(date_list.index('9/1/14'))
    x_axis.append(date_list.index('1/1/15'))
    ...
    if logarithmic:
        ...
        plt.gca().set_yticklabels(
            ['0', '10', '20', '30', '40', '50', '60', '70'], size='x-large')
    ...
    plt.title(
        'Total number of clicks: ' + str(np.sum(myarray)), size='x-large')
    plt.gca().set_xticks(x_axis)
    plt.gca().set_xticklabels(['Jan 12', 'May 12', 'Sep 12', 'Jan 13',
                               'May 13', 'Sep 13', 'Jan 14', 'May 14',
                               'Sep 14', 'Jan 15'], size='x-large')

    ## Exams  ##

    def make_label(date, course_name, height=65000, offset=5):
        index = date_list.index(date)
        plt.plot([index, index], [1, 70000], 'k:')
        if showExamLabels:
            plt.text(index + offset, height, course_name, size='x-large')

    make_label('12/5/14', 'MATH100', offset=-90)
#    make_label('12/6/14', 'MATH101')
    make_label('12/6/14', 'MATH102', 50000, offset=-90)
    make_label('12/6/14', 'MATH104', 35000, offset=-90)
    make_label('12/6/14', 'MATH105', 20000, offset=-90)
    #make_label('12/5/14', 'MATH110')
    #make_label('12/6/14', 'MATH200')
    make_label('12/15/14', 'MATH215', 42500)
#    make_label('12/2/14', 'MATH220')
    make_label('12/17/14', 'MATH221', 57500)
#    make_label('12/6/14', 'MATH257')
    #make_label('12/13/14', 'MATH307')
    #make_label('12/10/14', 'MATH312')
#    make_label('12/8/14', 'MATH437')
